...
void		Bureaucrat::signForm(Form &form) {
	if (form.is_Signed() && this->getGrade() <= form.getGradeSign()) {
		std::cout << this->getName() << " signed " << form.getName() << std::endl;
	}
	else {
		std::cout << this->getName() << " couldn't sign because Form requires " <<
		form.getGradeSign() << " and " << this->getName() << " has Grade " << this->getGrade() << std::endl;
	}
}